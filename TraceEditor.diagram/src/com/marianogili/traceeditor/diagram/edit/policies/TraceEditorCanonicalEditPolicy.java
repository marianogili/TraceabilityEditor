...
import com.marianogili.traceeditor.diagram.part.TraceEditorLinkDescriptor;
...
public class TraceEditorCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected void refreshSemantic() {
		...
		for (Iterator<TraceEditorNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			TraceEditorNodeDescriptor next = descriptorsIterator.next();
			String hint = TraceEditorVisualIDRegistry.getType(next
					.getVisualID());
		...
		for (TraceEditorNodeDescriptor next : childDescriptors) {
			String hint = TraceEditorVisualIDRegistry.getType(next
					.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<TraceEditorLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = TraceEditorVisualIDRegistry
			...
			for (Iterator<TraceEditorLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				TraceEditorLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<TraceEditorLinkDescriptor> collectAllLinks(View view,
			Domain2Notation domain2NotationMap) {
		if (!TraceEditorEditPart.MODEL_ID.equals(TraceEditorVisualIDRegistry
				.getModelID(view))) {
		...
		case TraceEditorEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(TraceEditorDiagramUpdater
						.getTraceEditor_1000ContainedLinks(view));
			}
		...
		case DashboardEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(TraceEditorDiagramUpdater
						.getDashboard_2001ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case ArtefactEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(TraceEditorDiagramUpdater
						.getArtefact_3001ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case TraceLinkEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(TraceEditorDiagramUpdater
						.getTraceLink_3002ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case TransformationEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(TraceEditorDiagramUpdater
						.getTransformation_3003ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case TraceLink2EditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(TraceEditorDiagramUpdater
						.getTraceLink_3005ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
		case Artefact2EditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(TraceEditorDiagramUpdater
						.getArtefact_3004ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
	...
	private Collection<IAdaptable> createConnections(
			Collection<TraceEditorLinkDescriptor> linkDescriptors,
			Domain2Notation domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (TraceEditorLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					TraceEditorVisualIDRegistry.getType(nextLinkDescriptor
	...
	protected final EditPart getHintedEditPart(EObject domainModelElement,
		...
		View view = (View) domain2NotationMap.getHinted(domainModelElement,
				TraceEditorVisualIDRegistry.getType(hintVisualId));
		if (view != null) {
			return (EditPart) getHost().getViewer().getEditPartRegistry()
					.get(view);
		}
