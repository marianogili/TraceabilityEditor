...
Uno de los desafíos presentados en el capítulo \ref{cap:ProblemasDesafios} (sección \ref{sec:SemanticaTracelinks}) refiere a la definición semántica de los \textit{tracelinks}, tarea que nos permitirá entender y distinguir los distintos tipos de \textit{links} con lo que nos podemos llegar a encontrar. Por ejemplo, un \textit{link} que muestra la relación entre un requerimiento textual y un elemento de un diagrama de caso de uso \glssymbol{uml} tiene una semántica distinta que un \textit{tracelink} que determina una relación de refinamiento dentro de un modelo de clases también \glssymbol{uml}.

Los tipos de \textit{tracelinks} por lo general son fuertemente dependientes al proyecto, por lo cual definir fijamente los tipos de enlaces semánticos tiene la consecuencia de menor flexibilidad para los enlaces que desee trazar el usuario, los cuales serán definidos por las necesidades del proyecto o la compañía.
