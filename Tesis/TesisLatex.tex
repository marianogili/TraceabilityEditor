...
%%%%     Propuesta      %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Esquema de traceability propuesto}
\label{cap:Propuesta}

En este capítulo se introduce, desarrolla y fundamenta el esquema de traceability propuesto en la presente tesis, el mismo se puede observar en el dibujo \ref{fig:EsquenaPropuesto}. 
...
La idea fundamental es lograr un meta-modelo bien simple en el que se capture solamente las ideas relevantes de traceability con lo cual mantener la independencia del dominio en el que se utilice y a su vez que capture la información mínima necesaria, dando principal énfasis en los siguientes desafíos:

\begin{itemize}

\item Un esquema que represente todas las trazas independientemente de como fueron generadas, si de forma explícita o implícitas, su granularidad y semántica.

\item No acomplejar la comunicación, tanto entre los distintos roles de usuarios como los distintos tipos de herramientas que hacen uso de traceability.

\item Que la adecuación de una herramienta que quiera agregar la funcionalidad de traceability mediante el uso de este esquema sea una tarea sencilla.

\item Aún dada la infinidad de artefactos que se pueden presentar, que el esquema ofrezca información semántica de los mismos, con el fin de facilitar la validación de enlaces legítimos.

\item Posibilidad de identificar unívocamente cada artefacto que se desee tracear.

\end{itemize}

El meta-modelo se encuentra lógicamente dividido en dos partes principales, la primordial en la que se encuentran las abstracciones de los elementos fundamentales necesarios en cualquier herramienta que desee ofrecer traceability, y una secundaria en la que se encierra la configuración del esquema. Esta configuración permite ofrecer un meta-modelo flexible a cualquier escenario/dominio en el que se desee hacer uso de traceability.
...
A continuación se listan un conjunto de aspectos/funcionalidades que el esquema actualmente definido no ofrece/tiene en cuenta:
