\documentclass[a4paper,12pt,twoside,spanish,openright]{book}
\usepackage[utf8]{inputenc}
\usepackage[dvipsnames]{xcolor}
\usepackage{babel,amsmath,graphicx,enumitem,amsfonts,appendix,listingsutf8,framed,parskip,fancyhdr}
...
\newglossaryentry{omg}{
	...
	symbol={OMG},
	description={Consorcio internacional de la industria informática sin fines de lucro y de membresía abierta responsable de la definición de varios estándares de modelado como \gls{uml}, \gls{mof} y \glssymbol{mda}}
}
...
\newglossaryentry{pde}{
	...
	symbol={PDE},
	description={es un ambiente de construcción de plug-ins Eclipse confeccionado por un conjunto de herramientas para crear, desarrollar, probar, depurar, construir y desplegar plug-ins, fragmentos, características, sitios de actualización y otros productos Eclipse}
}
...
\mtcaddchapter
\cleardoublepage %\clearpage %for openright
...
La definición anterior es utilizada principalmente por \textit{The Requirements Management Community}. En cambio, desde un punto de vista más cercano al contexto de \glssymbol{mdd}, se utiliza al término \textit{traceability} para describir 
\begin{quote}
...
traceability} propuesto en este trabajo de tesis. Del mismo modo se expone el prototipo de la herramienta construida que acompaña y ejemplifica las ideas del esquema presentado. Dicha herramienta, desarrollada como un \textit{\textbf{plug-in}} del \glssymbol{ide} \textsf{Eclipse}, asiste en la \textit{generación de tracelinks} sobre transformaciones de modelos existentes, y luego, presenta un mapa en el que se muestra la manera en que se relacionan las transformaciones, los \textit{links} y los artefactos orígenes y resultados de la ejecución de las transformaciones.
\end{quotation}
...
En el diagrama de componentes que podemos ver en la imagen \ref{fig:GMFArquitectura} se muestran las dependencias existentes entre el editor gráfico generado, el entorno \textsf{GMF Runtime}, \gls{emf}, \gls{gef} y la plataforma \textsf{Eclipse}. Como se puede ver, el editor gráfico \gls{gmf} depende del componente \textsf{GMF Runtime}, y a su vez hace uso directo de \gls{emf}, \gls{gef} y la plataforma \textsf{Eclipse}.
...
En el campo de \glssymbol{mde}, \gls{atl} ofrece un medio para especificar la forma de producir un conjunto de modelos resultados/destinos a partir de un conjunto de modelos fuentes.
...
\gls{qvt}, como ya se mencionó antes, es el estándar de \gls{omg} para la definición de transformaciones. La especificación de \gls{qvt} es híbrida, como en el caso de \gls{atl}, relacional (o declarativa) y operacional (o imperativa). Comprende tres diferentes lenguajes \gls{mmt}: dos lenguajes declarativos llamados \textsf{Relations} y \textsf{Core}, y un tercer lenguaje, de naturaleza imperativa, llamado \textsf{Operational Mappings}. Puede observarse la relación de los meta-modelos de cada uno de los lenguajes en la imagen \ref{fig:QVTmetamodelos}. 
...
La arquitectura se puede apreciar en la imagen \ref{fig:ArqETraceTool} y se explica de la siguiente manera, durante la transformación el \textit{plug-in} captura un conjunto de eventos previamente identificados y clasificados gracias a la programación orientada a aspectos, luego el \textsf{Aspect Tracer} genera un modelo de \textit{tracelinks} que conforma al \textit{meta-modelo de tracelinks} anidado que se muestra en la imagen \ref{fig:ETraceToolMeta-modelo}. Al final, el modelo de \textit{tracelinks} generado puede ser serializado en un archivo \gls{xmi} o transformado a cualquier otro lenguaje mediante una transformación modelo-texto.
...
Como se muestra en la imagen \ref{fig:SPLArquitectura}, la arquitectura ha sido definida principalmente en término de cuatro módulos. Cada uno de los cuales implementa una de las funcionalidades más importantes del framework. Dichos módulos se detallan a continuación:
...
\section{TiE - Integración de herramientas Case}
...
Para la asignación de tipos a los artefactos y a los enlaces se usa el concepto de faceta, donde \textsf{Trace-DSL} asigna un conjunto de facetas (\textsf{Facet}) a cada uno de los mismos. Un ejemplo de faceta se da en la imagen \ref{fig:FacetaCodigoFuente}. Además de lograr una solución simple al tipado de artefactos y enlaces, se obtiene un mecanismo fácilmente extensible y configurable al contexto donde se necesite aplicar \textit{traceability}.
...
\label{sec:TraceabilityLocalGlobal}

En la propuesta presentada en \cite{GlitiaEtienDumoulin} se emplea la idea de separación del proceso de \textit{traceability} en los siguientes niveles: \textit{traceability} en lo pequeño y \textit{traceability} en lo grande refiriéndose a los mismos como \textit{\textbf{traceability local}} y \textit{\textbf{traceability global}} respectivamente.

\subsection{Meta-modelo de Traceability Local}

Este meta-modelo toma los \textit{tracelinks} de la entrada y la salida de una única transformación. El meta-modelo está basado en el \textit{meta-modelo de tracelinks} presentado en \cite{Jouault} y se muestra en la imagen \ref{fig:LocalTraceMetamodel}.
...
El \textit{meta-modelo de tracelinks local} contiene dos conceptos principales, \textsf{Link} y \textsf{ElementRef}, los cuales expresan según su definición que uno o más elementos orígenes son enlazados a uno o más elementos objetivos. \textsf{ElementRef} es una clase abstracta que representa elementos que pueden ser \textit{traceados}: instancias de clases o valores de propiedades. Los valores de las propiedades son \textit{traceados} usando \textsf{PrimitivePropertyRef} el cual apunta a la instancia contenedora de la propiedad y tiene el nombre de la misma. Este tratamiento especial para los tipos primitivos de \textsf{Java} se debe a que no existen instancias de ellos en el modelo. Por otro lado las propiedades que son tipadas mediante una clase normal, son traceadas mediante \textsf{ClassRef}.
...
En la imagen \ref{fig:EjemploLocalGlobalMetamodel} se muestra un ejemplo de un modelo de \textit{tracelinks locales y globales} producto de una cadena de transformación. En el ejemplo se representan los enlaces (\textsf{Link}) entre los elementos sin tener en cuenta las instancias de \textsf{RuleRef} para no sobrecargarlo.
...
Este framework trabaja sobre un \textit{meta-modelo de tracelinks} que no tiene en cuenta las transformaciones de modelo que se usa en el paradigma de \glssymbol{mdd}, dado que el mismo fue pensado para el uso sobre familias de software independientemente de la metodología de desarrollo que utilizaron en cada una de ellas, posiblemente mediante la definición de un contexto especial se pueda lograr plasmar dicha información para el caso del paradigma \glssymbol{mdd}.
...
% encabezados
\lhead[Trabajos futuros]{}
\rhead[]{Trabajos futuros}

\chapter{Trabajos futuros}

\addcontentsline{toc}{section}{Lo que el esquema no ofrece}
...
\addcontentsline{toc}{section}{Mejoras para el prototipo}
...
\addcontentsline{toc}{section}{Para el futuro}
...
\item Mejorar la identificación semántica de un artefacto en la herramienta \mbox{\textsf{TraceEditor}}, con el fin de facilitar a una persona su reconocimiento.
...
% encabezados
\lhead[Anexos. Transformación QvtoTrace\_To\_Trace]{}
\rhead[]{Anexos. Transformación QvtoTrace\_To\_Trace}

\chapter{Definición de la transformación: QvtoTrace{\textunderscore}To{\textunderscore}Trace}
...
% operational: 
 ...
 commentstyle=\bfseries\itshape\color{OliveGreen},
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% encabezados
\lhead[Glosario y Siglas]{}
\rhead[]{Glosario y Siglas}
\printglossaries
