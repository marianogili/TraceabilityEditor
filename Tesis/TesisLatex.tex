...
\item Es muy importante para mantener la consistencia en \textit{traceability}, conocer y establecer el nivel de granularidad (o de ahora en más en inglés \textit{\textbf{trace granularity}}) de los elementos a ser enlazados. El problema es que no existe aún, un modelo claro de costo-beneficio que determine unívocamente cuál es el nivel de \textit{trace granularity} correcto.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Camino a la propuesta %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Camino a la propuesta}

En el presente capítulo nos introducimos en un conjunto de temas importantes de la teoría de \textit{traceability}, primero hablaremos sobre dos formas distintas de \textit{\textbf{generación de tracelinks}}, luego nos sumergiremos en el tema de la \textit{\textbf{semántica de los links}} y la compleja tarea de la definición de \textit{\textbf{tipos de tracelinks}}, y por último veremos dos \textit{\textbf{estrategias de almacenamiento}} principales de la información de \textit{traceability}. Estos temas han sido muy importantes y relevantes a la hora de la definición del \textit{esquema de traceability} propuesto que se detalla en el siguiente capítulo \ref{cap:Propuesta}.


\section{Tipos de tracelinks}

Uno de los desafíos presentados en el capítulo \ref{cap:ProblemasDesafios} (sección \ref{sec:SemanticaTracelinks}) refiere a la definición  \textit{semántica de los tracelinks}, tarea que nos permitirá entender y distinguir los distintos  \textit{tipos de links} con lo que nos podemos llegar a encontrar. Es importante remarcar que la determinación de la semántica de un \textit{link} se encuentra guiada por la razón del usuario respecto de qué quiere realizar o representar con dicho \textit{link}. No predefinir la \textit{semántica de un tracelink} correctamente puede resultar en fallas de razonamientos.

Por ejemplo, un \textit{link} que muestra la relación entre un requerimiento textual y un elemento de un diagrama de caso de uso \glssymbol{uml} tiene una semántica muy distinta a la de un \textit{tracelink} que determina una relación de refinamiento dentro de un modelo de clases también \glssymbol{uml}.

El conjunto de \textit{tipos de tracelinks} por lo general es fuertemente dependiente al contexto de un proyecto, por lo cual definirlo de forma fija tendrá como consecuencia una pérdida de flexibilidad para los usuarios que deseen definir los \textit{tracelinks} solamente de acuerdo a las necesidades del proyecto o la compañía.
