...
Por otro lado, el \gls{ide} \gls{atl} provee un conjunto de herramientas estándar (resaltado de sintaxis, auto-completado de código, depurador, etc) con el fin de facilitar el desarrollo de las transformaciones mediante este lenguaje.



\subsection{Algunos conceptos}

Un modelo fuente se transforma en un modelo destino gracias a la definición de una transformación escrita en \gls{atl}, la cual también es un modelo. Los modelos fuente, destino y la definición de la transformación, cada uno conforman a sus respectivos meta-modelos y, a su vez, todos los meta-modelos se ajustan a \gls{mof} o Ecore. Una transformación \gls{atl} es unidireccional, o sea que trabaja sobre un modelo fuente de solo lectura y produce un modelo destino de solo escritura. Durante la ejecución de una transformación, el modelo fuente puede ser navegado pero no cambiado, en cambio el modelo destino no puede ser navegado.
