...
La idea fundamental es lograr un meta-modelo bien simple en el que se capture solamente las ideas relevantes de traceability con lo cual mantener la independencia del dominio en el que se utilice y a su vez que capture la información mínima necesaria, con lo cual prestar especial énfasis en facilitar los siguientes aspectos:

\begin{itemize}

\item El intercambio de información entre las distintas herramientas
...
Entre los elementos fundamentales se encuentra TraceLink que representa una traza cuya información que almacena es un nombre que sirva como identificador (name), un tipo con el cual se determina su semántica (type), una referencia optativa a la transformación que pertenece, y dos conjuntos de artefactos: el de artefactos orígenes (sources) y el de artefactos objetivos (targets).
